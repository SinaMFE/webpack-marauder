...
try {
	json = JSON.parse(fs.readFileSync(path.resolve(__dirname, 'minpic.json'), 'utf-8'));
	...
	image.toFile(item.path, (rs) => {
		...
		if (rs && /limit/.test(rs.message)) {
			...
			fn().then(function(val) {
				fs.appendFileSync(path.resolve(cwd, 'minpic.txt'), '\n' + val);
				fs.stat(path.resolve(cwd, 'dist/static/img/' + path.parse(item.path).base), function(err, docs){
					if(!err){
						exec("cp " + item.path + " " + path.resolve(cwd, 'dist/static/img/' + path.parse(item.path).base ))
					}
				})
...
async function fn(){
	for (let i = 0; i < list.length; i++) {
		arr.push(await new Promise(function(resolve) {
			let hash = crypto.createHash('md5');
